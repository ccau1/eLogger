angular.module(Constants.Module).directive('ngMoney', ["$parse", function ($parse) {
    function link(scope, el, attrs, ngModelCtrl) {

        var NUMBER_REGEXP = /^\s*(\-|\+)?(\d+|(\d*(\.\d*)))\s*$/;
        var DEFAULT_PRECISION = 2;
        var minVal, maxVal, precision = DEFAULT_PRECISION, lastValidViewValue;
        ...
        function updateValuePrecision() {
            ...
            if (!isValueValid(modelValue)) modelValue = 0.00;
        ...
        if (isDefined(attrs.min) || attrs.ngMin) {
            scope.$watch(function() { return attrs.min }, function(val) {
        ...
        if (isDefined(attrs.max) || attrs.ngMax) {
            ...
            scope.$watch(function() {return attrs.max }, function(val) {
                if (isDefined(val) && !isNumber(val)) {
                    val = parseFloat(val, 10);
                }
